...
export function useDrillAudio(options: UseDrillAudioOptions = {}): DrillAudio {
  ...
  const { workletUrl = '/audio/pitch-processor.js?v=articulation-2026-08-05' } = options;
  ...
  const startMicRecording = useCallback(() => {
    ...
    try {
      ...
      // Periodic chunks are more reliable than a single final blob on Safari
      // and prevent an interrupted final event from erasing the whole take.
      recorder.start(250);
    } catch {
  ...
  const stopMicRecording = useCallback((): Promise<void> => {
    ...
    if (recorder?.state === 'recording') {
      try {
        ...
        // Some Safari builds throw when stop is already queued; stop still
        // produces their final dataavailable event.
      }
      recorder.stop();
    } else {
      settleRecording();
    ...
    try {
      ...
      worklet.port.onmessage = (event: MessageEvent) => {
        ...
        if (data.type === 'note-onset') {
          ...
          // amplitude gate and multi-frame stability check still reject room
          // noise before these messages can reach the drill.
          if (Number.isFinite(data.clarity) && data.clarity < 0.22) return;
          if (Number.isFinite(data.consensus) && data.consensus < 0.45) return;
