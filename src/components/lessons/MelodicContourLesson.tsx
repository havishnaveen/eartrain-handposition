...
export function MelodicContourLesson(_props: any) {
  ...
  const targetDirection = useRef<string>("Up-Down");

  const playDemo1 = async () => {
    if (isPlaying) return;
    setIsPlaying(true);
    const events: SequenceEvent[] = [{ notes: [{ note: "C", octave: 4 }], duration: 1, gapAfter: 0 }, { notes: [{ note: "F", octave: 4 }], duration: 1, gapAfter: 0 }, { notes: [{ note: "D", octave: 4 }], duration: 1, gapAfter: 0.5 }];
  ...
  const playDemo2 = async () => {
    if (isPlaying) return;
    setIsPlaying(true);
    const events: SequenceEvent[] = [{ notes: [{ note: "G", octave: 4 }], duration: 1, gapAfter: 0 }, { notes: [{ note: "D", octave: 4 }], duration: 1, gapAfter: 0 }, { notes: [{ note: "F", octave: 4 }], duration: 1, gapAfter: 0.5 }];
    await playSequenceWithUI(events, setActiveNotes);
    setIsPlaying(false);
  };
  ...
  const generateQuiz = () => {
      ...
      targetDirection.current = isFirst ? "Up-Down" : "Down-Up";
      
      const events: SequenceEvent[] = isFirst ? [{ notes: [{ note: "C", octave: 4 }], duration: 1, gapAfter: 0 }, { notes: [{ note: "F", octave: 4 }], duration: 1, gapAfter: 0 }, { notes: [{ note: "D", octave: 4 }], duration: 1, gapAfter: 0.5 }] : [{ notes: [{ note: "G", octave: 4 }], duration: 1, gapAfter: 0 }, { notes: [{ note: "D", octave: 4 }], duration: 1, gapAfter: 0 }, { notes: [{ note: "F", octave: 4 }], duration: 1, gapAfter: 0.5 }];
    ...
    <div className="space-y-10">
      ...
      <div className="prose prose-orange dark:prose-invert max-w-none">
        <p className="text-lg md:text-xl leading-relaxed text-stone-700 dark:text-stone-300 mb-6">
          When notes are played in succession, they create a melody. The shape of this melody—whether it goes up, down, or changes direction—is called its contour. An Up-Down contour starts low, peaks in the middle, and goes back down. A Down-Up contour starts high, dips in the middle, and goes back up.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          ...
          <span className="font-bold text-lg">Audio Demo: Up-Down</span>
        </Button>
        ...
        >
          ...
          <span className="font-bold text-lg">Audio Demo: Down-Up</span>
        </Button>
      </div>
      ...
      <div className="mt-12 pt-8 border-t border-stone-200 dark:border-stone-800">
        ...
        <p className="text-stone-600 dark:text-stone-400 mb-6">Press play to hear a hidden melody, then identify if its contour is Up-Down or Down-Up.</p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <Button 
              ...
              variant="outline"
              disabled={isPlaying}
              className={`flex-1 sm:w-32 border-2 ${quizStatus === 'correct' && targetDirection.current === 'Up-Down' ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : quizStatus === 'wrong' && targetDirection.current === 'Up-Down' ? 'border-amber-500 bg-amber-50 text-amber-600' : 'hover:border-orange-500'}`}
              onClick={() => handleGuess("Up-Down")}
            >
              Up-Down
            </Button>
            <Button 
              ...
              variant="outline"
              disabled={isPlaying}
              className={`flex-1 sm:w-32 border-2 ${quizStatus === 'correct' && targetDirection.current === 'Down-Up' ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : quizStatus === 'wrong' && targetDirection.current === 'Down-Up' ? 'border-amber-500 bg-amber-50 text-amber-600' : 'hover:border-amber-500'}`}
              onClick={() => handleGuess("Down-Up")}
            >
              Down-Up
            </Button>
          </div>
        ...
        {quizStatus === "wrong" && (
           <div className="mt-4 text-amber-600 font-medium animate-in fade-in slide-in-from-top-2">
             ...
             <p className="text-sm">If you guessed {targetDirection.current === "Up-Down" ? "Down-Up" : "Up-Down"}, try listening again and comparing the notes.</p>
           </div>
        )}
