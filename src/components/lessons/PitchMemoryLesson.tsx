...
export function PitchMemoryLesson(_props: any) {
  ...
  const targetDirection = useRef<string>("Same");

  const playDemo1 = async () => {
    if (isPlaying) return;
    setIsPlaying(true);
    const events: SequenceEvent[] = [{ notes: [{ note: "E", octave: 4 }], duration: 1, gapAfter: 1.5 }, { notes: [{ note: "E", octave: 4 }], duration: 1, gapAfter: 0.5 }];
  ...
  const playDemo2 = async () => {
    if (isPlaying) return;
    setIsPlaying(true);
    const events: SequenceEvent[] = [{ notes: [{ note: "E", octave: 4 }], duration: 1, gapAfter: 1.5 }, { notes: [{ note: "F", octave: 4 }], duration: 1, gapAfter: 0.5 }];
    await playSequenceWithUI(events, setActiveNotes);
    setIsPlaying(false);
  };
  ...
  const generateQuiz = () => {
      ...
      targetDirection.current = isFirst ? "Same" : "Different";
      
      const events: SequenceEvent[] = isFirst ? [{ notes: [{ note: "E", octave: 4 }], duration: 1, gapAfter: 1.5 }, { notes: [{ note: "E", octave: 4 }], duration: 1, gapAfter: 0.5 }] : [{ notes: [{ note: "E", octave: 4 }], duration: 1, gapAfter: 1.5 }, { notes: [{ note: "F", octave: 4 }], duration: 1, gapAfter: 0.5 }];
    ...
    <div className="space-y-10">
      ...
      <div className="prose prose-orange dark:prose-invert max-w-none">
        <p className="text-lg md:text-xl leading-relaxed text-stone-700 dark:text-stone-300 mb-6">
          Pitch memory is the ability to remember a specific musical note after it has been played. This is crucial for recognizing melodies, tuning instruments, and singing in key. We'll practice remembering a reference note and comparing it to another note.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          ...
          <span className="font-bold text-lg">Audio Demo: Same Note</span>
        </Button>
        ...
        >
          ...
          <span className="font-bold text-lg">Audio Demo: Different Note</span>
        </Button>
      </div>
      ...
      <div className="mt-12 pt-8 border-t border-stone-200 dark:border-stone-800">
        ...
        <p className="text-stone-600 dark:text-stone-400 mb-6">Press play to hear a note, followed by a short silence, and then another note. Identify if the second note is the SAME as the first, or DIFFERENT.</p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <Button 
              ...
              variant="outline"
              disabled={isPlaying}
              className={`flex-1 sm:w-32 border-2 ${quizStatus === 'correct' && targetDirection.current === 'Same' ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : quizStatus === 'wrong' && targetDirection.current === 'Same' ? 'border-amber-500 bg-amber-50 text-amber-600' : 'hover:border-orange-500'}`}
            ...
            <Button 
              ...
              variant="outline"
              disabled={isPlaying}
              className={`flex-1 sm:w-32 border-2 ${quizStatus === 'correct' && targetDirection.current === 'Different' ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : quizStatus === 'wrong' && targetDirection.current === 'Different' ? 'border-amber-500 bg-amber-50 text-amber-600' : 'hover:border-amber-500'}`}
              onClick={() => handleGuess("Different")}
            >
              Different
            </Button>
          </div>
        ...
        {quizStatus === "wrong" && (
           <div className="mt-4 text-amber-600 font-medium animate-in fade-in slide-in-from-top-2">
             ...
             <p className="text-sm">If you guessed {targetDirection.current === "Same" ? "Different" : "Same"}, try listening again and comparing the notes.</p>
           </div>
        )}
