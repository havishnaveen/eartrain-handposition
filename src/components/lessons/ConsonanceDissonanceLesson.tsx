...
export function ConsonanceDissonanceLesson(_props: any) {
  ...
  const targetDirection = useRef<string>("Consonant");

  const playDemo1 = async () => {
    if (isPlaying) return;
    setIsPlaying(true);
    const events: SequenceEvent[] = [{ notes: [{ note: "C", octave: 4 }, { note: "G", octave: 4 }], duration: 2, gapAfter: 0.5 }];
  ...
  const playDemo2 = async () => {
    if (isPlaying) return;
    setIsPlaying(true);
    const events: SequenceEvent[] = [{ notes: [{ note: "C", octave: 4 }, { note: "C#", octave: 4 }], duration: 2, gapAfter: 0.5 }];
    await playSequenceWithUI(events, setActiveNotes);
    setIsPlaying(false);
  };
  ...
  const generateQuiz = () => {
      ...
      targetDirection.current = isFirst ? "Consonant" : "Dissonant";
      
      const events: SequenceEvent[] = isFirst ? [{ notes: [{ note: "C", octave: 4 }, { note: "G", octave: 4 }], duration: 2, gapAfter: 0.5 }] : [{ notes: [{ note: "C", octave: 4 }, { note: "C#", octave: 4 }], duration: 2, gapAfter: 0.5 }];
    ...
    <div className="space-y-10">
      ...
      <div className="prose prose-orange dark:prose-invert max-w-none">
        <p className="text-lg md:text-xl leading-relaxed text-stone-700 dark:text-stone-300 mb-6">
          In music, intervals (two notes played together) can be categorized as Consonant or Dissonant. Consonant intervals sound stable, pleasant, and resolved. Dissonant intervals sound tense, clashing, and unresolved, wanting to move to a consonant interval.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          ...
          <span className="font-bold text-lg">Audio Demo: Consonant</span>
        </Button>
        ...
        >
          ...
          <span className="font-bold text-lg">Audio Demo: Dissonant</span>
        </Button>
      </div>
      ...
      <div className="mt-12 pt-8 border-t border-stone-200 dark:border-stone-800">
        ...
        <p className="text-stone-600 dark:text-stone-400 mb-6">Press play to hear a two-note interval. Identify if it sounds CONSONANT (stable and pleasant) or DISSONANT (tense and clashing).</p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <Button 
              ...
              variant="outline"
              disabled={isPlaying}
              className={`flex-1 sm:w-32 border-2 ${quizStatus === 'correct' && targetDirection.current === 'Consonant' ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : quizStatus === 'wrong' && targetDirection.current === 'Consonant' ? 'border-amber-500 bg-amber-50 text-amber-600' : 'hover:border-orange-500'}`}
              onClick={() => handleGuess("Consonant")}
            >
              Consonant
            </Button>
            <Button 
              ...
              variant="outline"
              disabled={isPlaying}
              className={`flex-1 sm:w-32 border-2 ${quizStatus === 'correct' && targetDirection.current === 'Dissonant' ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : quizStatus === 'wrong' && targetDirection.current === 'Dissonant' ? 'border-amber-500 bg-amber-50 text-amber-600' : 'hover:border-amber-500'}`}
              onClick={() => handleGuess("Dissonant")}
            >
              Dissonant
            </Button>
          </div>
        ...
        {quizStatus === "wrong" && (
           <div className="mt-4 text-amber-600 font-medium animate-in fade-in slide-in-from-top-2">
             ...
             <p className="text-sm">If you guessed {targetDirection.current === "Consonant" ? "Dissonant" : "Consonant"}, try listening again and comparing the notes.</p>
           </div>
        )}
