...
export function SecondsThirdsExercise({ onAnswer, showHint , onNext}: any) {
  const [isPlaying, setIsPlaying] = useState(false);
  const [showNext, setShowNext] = useState(false);
  const [hasPlayed, setHasPlayed] = useState(false);
  const [isRetry, setIsRetry] = useState(false);
  
  const currentInterval = useRef<{n1: {note: string, octave: number}, n2: {note: string, octave: number}, isThird: boolean} | null>(null);
  ...
  const generateQuestion = () => {
    ...
    // 2nd = 1 or 2 semitones. 3rd = 3 or 4 semitones.
    const semitones = isThird 
      ? (Math.random() > 0.5 ? 3 : 4) 
      : (Math.random() > 0.5 ? 1 : 2);
      
    const n2 = getNoteAtInterval(n1, semitones);
    
    currentInterval.current = { n1, n2, isThird };
    setIsRetry(false);
  };
  ...
  const submitAnswer = (guessIsThird: boolean) => {
    if (!currentInterval.current) return;
    const isCorrect = guessIsThird === currentInterval.current.isThird;
    if (isCorrect) {
      onAnswer(true);
    ...
    <div className="p-8 flex flex-col items-center relative overflow-hidden">
      ...
      {isRetry && currentInterval.current && (
        <MusicStaff notes={[currentInterval.current.n1, currentInterval.current.n2]} 
          caption={`The interval was a ${currentInterval.current.isThird ? "3rd" : "2nd"}`} 
        />
      )}
      ...
      {showHint && !isRetry && (
        <p className="text-orange-600 dark:text-orange-400 bg-orange-500/10 dark:bg-orange-500/10 p-4 rounded-xl text-sm font-medium mb-8 max-w-lg text-center">
          Hint: 2nds sound very dissonant and close together (like Jaws or Happy Birthday). 3rds sound more consonant and pleasing.
        </p>
      )}
