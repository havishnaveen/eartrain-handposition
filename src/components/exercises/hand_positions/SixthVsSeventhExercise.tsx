...
const SIXTHS = [
  [{ note: "C", octave: 4 }, { note: "A", octave: 4 }],
  [{ note: "D", octave: 4 }, { note: "B", octave: 4 }],
  [{ note: "E", octave: 4 }, { note: "C", octave: 5 }],
  [{ note: "F", octave: 4 }, { note: "D", octave: 5 }],
  [{ note: "G", octave: 4 }, { note: "E", octave: 5 }],
];

const SEVENTHS = [
  [{ note: "C", octave: 4 }, { note: "B", octave: 4 }],
  [{ note: "D", octave: 4 }, { note: "C", octave: 5 }],
  [{ note: "E", octave: 4 }, { note: "D", octave: 5 }],
  [{ note: "F", octave: 4 }, { note: "E", octave: 5 }],
  [{ note: "G", octave: 4 }, { note: "F", octave: 5 }],
];

type QuestionType = "6th" | "7th";
...
function MiniKeyboard({ highlightNotes }: { highlightNotes: number[] }) {
  return (
    <div className="relative flex bg-white p-1 rounded-md shadow-inner border border-slate-300">
      ...
      {/* Black keys for C position to C Octave */}
  ...
  return (
    <div className="flex flex-col gap-6 w-full p-6 bg-slate-800 rounded-2xl my-4 text-white shadow-xl border border-slate-700">
      <div className="text-center mb-2">
        ...
        <p className="text-slate-300">A 7th stretches almost to the octave and often sounds dissonant, while a 6th is a bit smaller and sweeter.</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8 justify-center">
        ...
        <div className="flex flex-col items-center bg-slate-700/50 p-6 rounded-xl border border-slate-600 flex-1 relative overflow-hidden">
          ...
          <h4 className="font-bold text-xl mb-4 text-green-400 flex flex-col items-center gap-2">
            <span>6th (Sweet stretch)</span>
            {targetType === "6th" && <span className="bg-green-500/20 text-green-400 font-bold uppercase tracking-widest text-xs px-2 py-1 rounded-md">Correct Answer</span>}
            {targetType === "7th" && <span className="bg-red-500/20 text-red-400 font-bold uppercase tracking-widest text-xs px-2 py-1 rounded-md">You Chose</span>}
          </h4>
          <MiniKeyboard highlightNotes={[0, 5]} />
          <p className="mt-4 text-sm text-slate-300 text-center font-medium">Spans 6 notes.<br/>(e.g. C to A)</p>
        </div>
        ...
        <div className="flex flex-col items-center bg-slate-700/50 p-6 rounded-xl border border-slate-600 flex-1 relative overflow-hidden">
          ...
          <h4 className="font-bold text-xl mb-4 text-blue-400 flex flex-col items-center gap-2">
            <span>7th (Tense stretch)</span>
            {targetType === "7th" && <span className="bg-green-500/20 text-green-400 font-bold uppercase tracking-widest text-xs px-2 py-1 rounded-md">Correct Answer</span>}
            {targetType === "6th" && <span className="bg-red-500/20 text-red-400 font-bold uppercase tracking-widest text-xs px-2 py-1 rounded-md">You Chose</span>}
          </h4>
          <MiniKeyboard highlightNotes={[0, 6]} />
          <p className="mt-4 text-sm text-slate-300 text-center font-medium">Spans 7 notes.<br/>(e.g. C to B, one step below the octave)</p>
        </div>
      </div>
    </div>
  );
...
export function SixthVsSeventhExercise({ onComplete }: Props) {
  const [targetType, setTargetType] = useState<QuestionType>("6th");
  ...
  const [step, setStep] = useState(() => {
    return localStorage.getItem('et_v3_seen_example_sixth-vs-seventh') ? 1 : 0;
  });
  const [examplePhase, setExamplePhase] = useState<'play' | 'guess' | 'none'>(() => {
    return localStorage.getItem('et_v3_seen_example_sixth-vs-seventh') ? 'none' : 'play';
  });
  ...
  const generateQuestion = (nextStep: number) => {
    ...
    let type: QuestionType = Math.random() > 0.5 ? "6th" : "7th";
    if (nextStep === 0) type = "6th"; // Example is always a 6th
    
    const pool = type === "6th" ? SIXTHS : SEVENTHS;
    ...
    if (nextStep === 0) {
      setMelody(SIXTHS[0]); // Force C4->A4 for example
    } else {
  ...
  const nextStep = () => {
    if (step === 0) {
      localStorage.setItem('et_v3_seen_example_sixth-vs-seventh', 'true');
    }
    const next = step + 1;
    if (next > MAX_STEPS) {
      onComplete();
  ...
  return (
    <ExerciseLayout
      title="Lesson 13: 6th vs 7th"
      step={step}
      maxSteps={MAX_STEPS}
      exampleInstruction="This is a 6th - a nice, consonant wide stretch. Listen closely."
      practiceInstruction="Is this interval a 6th or a 7th?"
      feedback={feedback}
      onNextStep={nextStep}
      onComplete={onComplete}
      storageKeyId="sixth-vs-seventh"
        ...
        <div className={`grid grid-cols-2 gap-6 w-full relative ${step === 0 && examplePhase === "guess" ? "z-50" : "z-10"}`}>
          <button
            ...
            className={`relative ${step === 0 && examplePhase === 'guess' ? 'z-50' : 'z-10'} 
              ...
              ${feedback === "success" && targetType === "6th" 
                ? "bg-green-500/20 border-green-500 text-green-400" 
                : feedback === "error" && targetType === "7th"
          ...
          <button
            onClick={() => handleGuess("7th")}
            disabled={isPlaying || feedback === "success" || (step === 0 && examplePhase === "play")}
            className={`relative ${step === 0 && examplePhase === 'guess' ? 'z-50' : 'z-10'} 
              flex flex-col items-center justify-center p-8 rounded-2xl border-2 transition-all
              ${feedback === "success" && targetType === "7th" 
                ? "bg-green-500/20 border-green-500 text-green-400" 
                : feedback === "error" && targetType === "6th"
                ? "bg-red-500/20 border-red-500 text-red-400 opacity-50"
                : "bg-slate-800 border-slate-700 text-white hover:bg-slate-700 hover:border-slate-500"}
            `}
          >
            ...
            <span className="text-xl font-bold">7TH</span>
          </button>
        </div>
      </div>
    </ExerciseLayout>
