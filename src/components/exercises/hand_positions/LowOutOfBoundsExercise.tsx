...
import { Play, ArrowDownRight, ShieldCheck } from "lucide-react";
...
// Different melody shapes for G Major position
const IN_BOUNDS_MELODIES = [
  [{ note: "G4", octave: 4 }, { note: "B4", octave: 4 }, { note: "D5", octave: 5 }, { note: "B4", octave: 4 }],
  [{ note: "G4", octave: 4 }, { note: "A4", octave: 4 }, { note: "B4", octave: 4 }, { note: "D5", octave: 5 }],
  [{ note: "D5", octave: 5 }, { note: "B4", octave: 4 }, { note: "A4", octave: 4 }, { note: "G4", octave: 4 }],
  [{ note: "B4", octave: 4 }, { note: "A4", octave: 4 }, { note: "G4", octave: 4 }, { note: "A4", octave: 4 }],
  [{ note: "G4", octave: 4 }, { note: "D5", octave: 5 }, { note: "B4", octave: 4 }, { note: "A4", octave: 4 }],
];

const OUT_OF_BOUNDS_MELODIES = [
  [{ note: "G4", octave: 4 }, { note: "B4", octave: 4 }, { note: "D5", octave: 5 }, { note: "F#4", octave: 4 }],
  [{ note: "G4", octave: 4 }, { note: "A4", octave: 4 }, { note: "F#4", octave: 4 }, { note: "E4", octave: 4 }],
  [{ note: "B4", octave: 4 }, { note: "G4", octave: 4 }, { note: "E4", octave: 4 }, { note: "G4", octave: 4 }],
  [{ note: "D5", octave: 5 }, { note: "B4", octave: 4 }, { note: "G4", octave: 4 }, { note: "D4", octave: 4 }],
  [{ note: "A4", octave: 4 }, { note: "G4", octave: 4 }, { note: "F#4", octave: 4 }, { note: "D4", octave: 4 }],
];

export function LowOutOfBoundsExercise({ onComplete }: Props) {
  ...
  const [step, setStep] = useState(() => {
    return localStorage.getItem('et_v3_seen_example_low-out-of-bounds') ? 1 : 0;
  });
  const [examplePhase, setExamplePhase] = useState<'play' | 'guess' | 'none'>(() => {
    return localStorage.getItem('et_v3_seen_example_low-out-of-bounds') ? 'none' : 'play';
  });
  const MAX_STEPS = 5;

  const noteOrder = ["C4", "D4", "E4", "F#4", "G4", "A4", "B4", "C5", "D5"];
  const outNote = melody.find(n => noteOrder.indexOf(n.note) < noteOrder.indexOf("G4"))?.note || "F#4";
  ...
  const nextStep = () => {
    if (step === 0) {
      localStorage.setItem('et_v3_seen_example_low-out-of-bounds', 'true');
    }
    if (step < MAX_STEPS) {
      const next = step + 1;
      setStep(next);
  ...
  return (
    <ExerciseLayout
      title="Lesson 8: Low Out-of-Bounds"
      step={step}
      maxSteps={MAX_STEPS}
      exampleInstruction="Now we're in the G Major position (G-A-B-C-D). Listen — does the melody stay inside those 5 notes, or does it dip DOWN below G to F#?"
      practiceInstruction="Listen carefully! Did the melody stay safely inside G–D (your 5 fingers), or did it sneak LOW below G down to F#?"
      feedback={feedback}
      onNextStep={nextStep}
      onComplete={onComplete}
      storageKeyId="low-out-of-bounds"
      ...
      <div className="flex flex-col items-center gap-4 min-h-32 mt-4 w-full max-w-lg mx-auto">
        {examplePhase === 'guess' && feedback === 'none' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-12 w-full max-w-sm relative z-50">
            <OutOfBoundsVisual outOfBoundsNote={outNote} direction="low" />
          </motion.div>
        )}
          ...
          <div className="flex gap-4 w-full">
            <div className={`relative flex-1 ${examplePhase === 'guess' ? 'z-50' : 'z-10'}`}>
              ...
              >
                ...
                <span className="text-sm opacity-80 uppercase tracking-widest">(Stayed G–D)</span>
              </button>
            </div>
            <div className={`relative flex-1 ${examplePhase === 'guess' ? 'z-50' : 'z-10'}`}>
              {examplePhase === 'guess' && <AnimatedPointer className="-top-12 left-1/2 -translate-x-1/2" />}
              <button 
                ...
                className={`relative ${step === 0 && examplePhase === 'guess' ? 'z-50' : 'z-10'} w-full py-4 bg-rose-500 text-white font-black text-xl rounded-2xl hover:bg-rose-600 active:scale-95 transition-all shadow-xl border-b-4 border-rose-700 flex flex-col items-center justify-center gap-1`}
              >
                <div className="flex items-center gap-2"><ArrowDownRight className="w-6 h-6" /> LOW DIP</div>
                <span className="text-sm opacity-80 uppercase tracking-widest">(Reached lower!)</span>
              </button>
            </div>
          </div>
        )}
