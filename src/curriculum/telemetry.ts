...
export interface AttemptRecord {
  ...
  at: number;
  /** Opaque learner id; legacy rows remain unscoped and are migrated once. */
  studentId?: string;
  launchId?: string;
  assignmentId?: string;
...
function isAttemptRecord(value: unknown): value is AttemptRecord {
  ...
  return (
    ...
    (record.launchId === undefined || typeof record.launchId === 'string') &&
    (record.assignmentId === undefined || typeof record.assignmentId === 'string') &&
    (record.primaryProblem === undefined || typeof record.primaryProblem === 'string') &&
    (record.problemTags === undefined || (
      Array.isArray(record.problemTags) &&
      record.problemTags.every((problem) => typeof problem === 'string')
    )) &&
...
class TelemetryStore {
  ...
  record(entry: Omit<AttemptRecord, 'seq'>): AttemptRecord {
    const activeStudent = learningProfileStore.getActiveStudent();
    const full: AttemptRecord = {
      ...entry,
      studentId: entry.studentId ?? activeStudent.id,
      seq: ++this.seq,
    };
