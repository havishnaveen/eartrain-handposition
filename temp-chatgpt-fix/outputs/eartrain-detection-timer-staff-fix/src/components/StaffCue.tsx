  ...
  useEffect(() => {
    ...
    const maxTimelineWidth = cue.staves.reduce(
      (largest, staff) => {
        ...
        const collisionSafe = (total / shortestStep) * MIN_PER_NOTE;
    ...
    cue.staves.forEach((staffSpec: StaffSpec, staffIndex: number) => {
      ...
      if (staffIndex === 0) {
        ...
        layout = {
          points,
          startX: timeline?.startX ?? points[0]?.x ?? noteStart,
          endX: timeline?.endX ?? timelineEnd,
          totalBeats: timeline?.totalBeats ?? Math.max(beat, points.length),
