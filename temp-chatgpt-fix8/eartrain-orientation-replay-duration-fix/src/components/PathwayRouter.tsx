...
function orientationNoticesFor(question: Question): OrientationNotice[] {
  ...
  if (handScope === 'left') {
    notices.push({
      kind: 'left-hand',
      title: 'Hand change',
      message: 'Use your left hand now. Look for LH.',
    });
  } else if (handScope === 'both') {
    notices.push({
      kind: 'both-hands',
      title: 'Both hands now',
      message: 'This lesson uses RH and LH. Check the hand label each time.',
    });
  }
  return notices;
}
...
type PathwayAction =
  ...
  | { type: 'MEMORY_START' }
  | { type: 'START' }
  | { type: 'PLAY_START'; now: number }
  | { type: 'ANALYSIS_START'; now: number }
  | { type: 'RESOLVE'; result: GradeResult; detected: readonly DetectedNote[]; now: number }
  | { type: 'CONTINUE'; difficultyNudge: number };

const clampDifficulty = (difficulty: number): number =>
  Math.min(1, Math.max(0, difficulty));

function initialStatusFor(question: Question): ExerciseStatus {
  return question.exerciseMode === 'prove-it' ? 'position-prompt' : 'prompt';
}
...
function generateFor(
  ...
  return concept.generate(ordinal, rand, difficulty, nextMode(signal));
}

function initialState({ seed, cap }: { seed: number; cap: number }): PathwayState {
...
function reducer(state: PathwayState, action: PathwayAction): PathwayState {
  if (state.finished) return state;

  switch (action.type) {
    case 'PROOF_START':
      return state.status === 'position-prompt' ? { ...state, status: 'proving' } : state;

    case 'PROOF_SUCCESS':
      return state.status === 'proving' ? { ...state, status: 'proof-success' } : state;

    case 'PROOF_CANCEL':
      return state.status === 'proving' ? { ...state, status: 'position-prompt' } : state;

    case 'PROOF_UNLOCK':
      return state.status === 'proof-success' ? { ...state, status: 'prompt' } : state;

    case 'MEMORY_START':
      return state.status === 'prompt' && state.current.exerciseMode === 'blind-memory'
        ? { ...state, status: 'memory-preview' }
        : state;

    case 'START':
      return state.status === 'prompt' || state.status === 'memory-preview'
        ? { ...state, status: 'leadin' }
        : state;
    ...
    case 'PLAY_START':
      ...
      if (state.status !== 'leadin') return state;
    ...
    case 'CONTINUE': {
      ...
      const base = {
        ...state,
        status: initialStatusFor(state.current),
        report: null,
        performedNotes: [],
        repeatQuestion: false,
        analysisStartedAt: 0,
...
export function PathwayRouter({
  ...
  const [state, dispatch] = useReducer(reducer, { seed, cap: sessionQuestionCap }, initialState);
  ...
  const handlePlayStart = useCallback((audioTime: number) => {
    ...
    dispatch({ type: 'PLAY_START', now: Date.now() });
  }, []);
  ...
  const handleAnalysisStart = useCallback(() => {
    ...
    dispatch({ type: 'ANALYSIS_START', now });
  }, []);

  const handleFinish = useCallback((detected: DetectedNote[]) => {
  ...
  const startActualDrill = useCallback(() => {
    const plan = planForQuestion(questionRef.current, bpm);
  ...
  const handleStart = useCallback(() => {
    ...
    if (active.exerciseMode === 'prove-it' && active.positionProof && state.status === 'position-prompt') {
      ...
      void beginProofRef.current(active.positionProof)
        .then((started) => {
          ...
          if (started) dispatch({ type: 'PROOF_START' });
        })
        .catch(() => {
          proofStartingRef.current = false;
        });
    ...
    startActualDrill();
  }, [clearMemoryTimers, startActualDrill, state.status]);
  ...
  useEffect(() => {
    if (state.finished) abortRef.current();
  }, [state.finished]);
