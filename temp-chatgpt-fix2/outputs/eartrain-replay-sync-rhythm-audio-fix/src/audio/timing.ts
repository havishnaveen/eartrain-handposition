...
function buildRhythm(
  ...
  // Absolute onsets catch playing the whole phrase late or early. Interval
  // errors independently catch rushing, dragging, and uneven note spacing,
  // including cases whose early and late attacks average out around zero.
  ...
  const combinedError = meanOnsetError + meanIntervalError * 0.35;
...
export function gradeSequence(
  ...
  const baseTimingScore =
    rhythm === null
      ? null
      : clamp5(
          5 *
            Math.pow(
              Math.max(0, 1 - rhythm.meanAbsBeats / timingProfile.zeroScoreWindow),
              1.35,
            ),
        );
  const timingScore =
