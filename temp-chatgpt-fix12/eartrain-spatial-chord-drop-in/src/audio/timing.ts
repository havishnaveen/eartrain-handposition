...
/**
 * Grade a root-first spatial chord search.
 *
 * This deliberately does not reuse beat alignment: the musical task is to
 * locate a harmonic anchor and complete its physical shape, not to perform a
 * notated rhythm. Search speed becomes Timing, and unnecessary deliberate
 * guesses become Cleanliness. Detector candidates/echoes never enter the
...
export function gradeSpatialChord(
  ...
  const shapeSpeed = buildLatencySec === null
    ? 0
    : Math.max(0, 1 - buildLatencySec / Math.max(1, spec.shapeSearchSeconds));
  const guessPenalty =
    performance.wrongRootGuesses * 0.55 + performance.wrongShapeGuesses * 0.4;
  const efficiencyScore = scoreToFive(
    5 * (rootSpeed * 0.52 + shapeSpeed * 0.33 + shapeCoverage * 0.15) - guessPenalty,
  );
  const pitchScore = scoreToFive(expectedCount === 0 ? 0 : 5 * matched / expectedCount);
  const cleanlinessScore = detected.length === 0
    ? 0
    : scoreToFive(
        5 - performance.wrongRootGuesses * 1.05 - performance.wrongShapeGuesses * 0.78,
      );
  ...
  const passed =
    completed &&
    performance.wrongRootGuesses + performance.wrongShapeGuesses <= spec.maxWrongGuesses;

  let detail: string;
  if (passed && performance.wrongRootGuesses === 0 && performance.wrongShapeGuesses === 0) {
    detail = 'You found the root and built the chord without guessing.';
  } else if (passed) {
    detail = 'Chord complete. Try to reach the same shape with fewer guesses next time.';
  } else if (!performance.rootFound) {
    detail = 'The root was not found. Listen for the lowest, most settled piano note.';
  } else if (!completed) {
...
export function gradeSequence(
  ...
  const passed = pitchAndCleanlinessPassed && transitionPassed;
