...
function supportsExpectedPitch(
  ...
  const minimumFrames = strictPrimary ? 4 : 2;
  const minimumConsensus = strictPrimary ? 0.62 : 0.45;
  const maximumMad = strictPrimary ? 0.42 : 0.62;
  const minimumClarity = hypothesis.source === 'yin'
    ? (strictPrimary ? 0.5 : 0.32)
    : (strictPrimary ? 0.22 : 0.15);
  return (
...
export function resolveContextualPitch(
  ...
  const primarySlot = findRecoverableExpectedSlot(
    plan,
    playStartTime,
    primaryMidi,
    onsetTime,
...
export function findRecoverableExpectedSlot(
  ...
  plan.expectedNotes.forEach((slot, index) => {
    ...
    const error = Math.abs(onsetBeat - slot.beat);
    // Longer notes receive a little more arrival grace, while never opening
    // a window wide enough to steal the following note's slot.
    const windowBeats = Math.min(1.1, Math.max(0.72, 0.48 + slot.beats * 0.24));
    if (error <= windowBeats && error < bestError) {
      bestIndex = index;
      bestError = error;
    }
  });
...
const RETRIGGER_PREVIOUS_PEAK_RATIO = 0.22;
const LATE_RETRIGGER_ATTACK_RATIO = 1.08;
...
export function isClearSamePitchRetrigger(
  ...
  const freshPhysicalAttack = hasArticulationMetrics
    ? (
        ((current.frameAttackRatio as number) >= 1.08 && (current.novelty as number) >= 0.24) ||
        ((current.frameAttackRatio as number) >= 0.97 && (current.novelty as number) >= 0.35)
      )
    : Number.isFinite(current.attackRatio) && current.attackRatio >= LATE_RETRIGGER_ATTACK_RATIO;
  const freshEnvelope =
    freshPhysicalAttack &&
...
/**
 * Root-first chord search with monotonic progress.
 *
 * Already-found chord tones are harmless repeats. New remaining chord tones
 * may be supplied in either order because the goal is the physical shape,
 * not memorising an arbitrary third-then-fifth sequence. Every other strict
 * onset is a guess and is retained for efficiency scoring.
 */
export function advanceSpatialChord(
  ...
  const root = active.targetMidi[0];
  ...
  if (active.targetMidi.includes(midi)) {
...
export function useDrillAudio(options: UseDrillAudioOptions = {}): DrillAudio {
  ...
  const { workletUrl = '/audio/pitch-processor.js?v=ensemble-preroll-2026-08-06' } = options;
  ...
  }, [stopLoops, stopSpatialPlayback, cancelMicRecording]);
    ...
    try {
      ...
      worklet.port.onmessage = (event: MessageEvent) => {
        ...
        if (data.type === 'note-onset' || data.type === 'note-candidate') {
          ...
          // gate with every onset. The recovery lane may sit slightly below
          // that gate, but never below 82% and never without a score match.
          if (
            Number.isFinite(data.peakRms) &&
            Number.isFinite(data.gate) &&
            data.peakRms < data.gate * (isCandidate ? 0.82 : 1)
          ) {
            if (isCandidate) diagnosticsRef.current.candidatesIgnored += 1;
            return;
          }
          if (Number.isFinite(data.clarity) && data.clarity < (isCandidate ? 0.16 : 0.25)) return;
          if (Number.isFinite(data.consensus) && data.consensus < (isCandidate ? 0.34 : 0.5)) return;
          ...
          if (spatialRef.current) {
            ...
            const wanted = active.rootFoundAt === null
              ? [active.targetMidi[0]]
              : active.targetMidi.filter((targetMidi) => !active.foundMidi.has(targetMidi));
            ...
            if (isCandidate && !wanted.includes(midi)) {
          ...
          if (proofRef.current) {
            ...
            if (midi !== wanted && isCandidate) {
              diagnosticsRef.current.candidatesIgnored += 1;
              return;
            }
            safeSet(setDetectedNames, [midiToName(midi)]);
            if (isCandidate) diagnosticsRef.current.expectedRecovered += 1;
            else diagnosticsRef.current.strictAccepted += 1;
  ...
  const scheduleSpatialContext = useCallback((ctx: AudioContext, spec: SpatialChordSpec): number => {
    const bus = ctx.createGain();
    bus.gain.value = 0.72;
    bus.connect(ctx.destination);
    const track = (source: AudioScheduledSourceNode, cleanup: AudioNode[]) => {
      spatialSourcesRef.current.add(source);
      source.onended = () => {
        spatialSourcesRef.current.delete(source);
    ...
    const frequencyForPitch = (pitch: string, shift = 0) => {
      const midi = pitchToMidi(pitch);
      return midi === null ? 261.63 : 440 * Math.pow(2, (midi + shift - 69) / 12);
    };
    const schedulePiano = (pitches: string[], time: number, duration: number) => {
      pitches.forEach((pitch, noteIndex) => {
        [
          { type: 'triangle' as OscillatorType, ratio: 1, gain: 0.095 },
          { type: 'sine' as OscillatorType, ratio: 2, gain: 0.025 },
        ].forEach((voice, voiceIndex) => {
          const oscillator = ctx.createOscillator();
          const envelope = ctx.createGain();
          const pan = ctx.createStereoPanner();
          oscillator.type = voice.type;
          oscillator.frequency.value = frequencyForPitch(pitch) * voice.ratio;
          oscillator.detune.value = voiceIndex === 0 ? -2 : 3;
          pan.pan.value = (noteIndex - 1) * 0.08;
          envelope.gain.setValueAtTime(0.0001, time);
          envelope.gain.exponentialRampToValueAtTime(voice.gain, time + 0.012);
          envelope.gain.exponentialRampToValueAtTime(voice.gain * 0.32, time + 0.24);
          envelope.gain.exponentialRampToValueAtTime(0.0001, time + duration);
          oscillator.connect(envelope);
          envelope.connect(pan);
          pan.connect(bus);
          track(oscillator, [envelope, pan]);
          oscillator.start(time);
          oscillator.stop(time + duration + 0.03);
        });
      });
    };
    const schedulePad = (pitches: string[], time: number, duration: number) => {
      pitches.slice(0, 2).forEach((pitch, index) => {
        const oscillator = ctx.createOscillator();
        const filter = ctx.createBiquadFilter();
        const envelope = ctx.createGain();
        const pan = ctx.createStereoPanner();
        oscillator.type = 'sine';
        oscillator.frequency.value = frequencyForPitch(pitch, index === 0 ? -12 : 0);
        oscillator.detune.value = index === 0 ? -7 : 8;
        filter.type = 'lowpass';
        filter.frequency.value = 850;
        pan.pan.value = index === 0 ? -0.65 : 0.65;
        envelope.gain.setValueAtTime(0.0001, time);
        envelope.gain.linearRampToValueAtTime(0.027, time + 0.22);
        envelope.gain.linearRampToValueAtTime(0.0001, time + duration);
        oscillator.connect(filter);
        filter.connect(envelope);
        envelope.connect(pan);
        pan.connect(bus);
        track(oscillator, [filter, envelope, pan]);
        oscillator.start(time);
        oscillator.stop(time + duration + 0.03);
      });
    };
    const scheduleBass = (root: string, time: number, duration: number) => {
      const oscillator = ctx.createOscillator();
      const envelope = ctx.createGain();
      const filter = ctx.createBiquadFilter();
      oscillator.type = 'sine';
      oscillator.frequency.value = frequencyForPitch(root, -12);
      filter.type = 'lowpass';
      filter.frequency.value = 360;
      envelope.gain.setValueAtTime(0.0001, time);
      envelope.gain.exponentialRampToValueAtTime(0.07, time + 0.025);
    ...
    const scheduleStrings = (pitch: string, time: number, duration: number) => {
      const oscillator = ctx.createOscillator();
      const filter = ctx.createBiquadFilter();
      const envelope = ctx.createGain();
      const pan = ctx.createStereoPanner();
      oscillator.type = 'sawtooth';
      oscillator.frequency.value = frequencyForPitch(pitch, 12);
      filter.type = 'lowpass';
      filter.frequency.value = 1200;
      pan.pan.value = 0.72;
      envelope.gain.setValueAtTime(0.0001, time);
      envelope.gain.linearRampToValueAtTime(0.012, time + 0.18);
      envelope.gain.linearRampToValueAtTime(0.0001, time + duration);
      oscillator.connect(filter);
      filter.connect(envelope);
      envelope.connect(pan);
    ...
    const schedulePulse = (time: number) => {
      ...
      envelope.connect(pan);
      pan.connect(bus);
      track(source, [filter, envelope, pan]);
      source.start(time);
    ...
    const sequence = [
      ...
      ...Array.from(
        { length: Math.max(0, spec.context.targetRepeats - 1) },
        () => spec.context.progression[spec.context.targetChordIndex] ?? spec.chordPitches,
      ),
    ];
    const start = ctx.currentTime + 0.16;
    sequence.forEach((chord, index) => {
      const time = start + index * chordDuration;
      schedulePiano(chord, time, chordDuration * 0.92);
      if (spec.context.layers.includes('pad')) schedulePad(chord, time, chordDuration * 0.96);
      if (spec.context.layers.includes('bass')) scheduleBass(chord[0], time, chordDuration);
      if (spec.context.layers.includes('strings')) scheduleStrings(chord[2] ?? chord[0], time, chordDuration * 0.96);
      if (spec.context.layers.includes('pulse')) {
        schedulePulse(time);
        schedulePulse(time + chordDuration * 0.5);
      }
    });
    const endTime = start + sequence.length * chordDuration;
    window.setTimeout(() => {
      try { bus.disconnect(); } catch { /* already disconnected */ }
    }, Math.max(0, (endTime - ctx.currentTime + 0.8) * 1000));
    return endTime;
  }, []);
  ...
  }, [stopLoops, stopSpatialPlayback, cancelMicRecording, safeSet]);
  ...
  const beginSpatialChord = useCallback(
    async (target: SpatialChordSpec): Promise<boolean> => {
      ...
      const clueEndTime = scheduleSpatialContext(ctx, target);
