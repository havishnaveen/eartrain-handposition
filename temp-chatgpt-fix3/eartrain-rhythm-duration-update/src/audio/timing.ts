import type { AnchorShiftSpec, CueSpec, Question } from '../curriculum/types';
...
function buildRhythm(
  ...
  // Absolute onsets catch playing the whole phrase late or early. The gap
  // from one confirmed attack to the next is the playable acoustic measure
  // of the first note's written duration (plus any written rest). A piano
  // string can ring long after the key is released, especially under pedal,
  // so amplitude decay is deliberately NOT treated as key-up time.
  ...
  for (let index = 1; index < matches.length; index++) {
    ...
    intervalErrors.push(Math.abs(playedGap - expectedGap));
  }
  ...
  return {
    ...
    durationEvaluated: intervalErrors.length,
    durationAccuracy: durationAccuracy === null
      ? null
      : Math.round(durationAccuracy * 1000) / 1000,
    meanDurationErrorBeats: intervalErrors.length === 0
      ? null
      : Math.round(meanIntervalError * 100) / 100,
  };
}
...
export function gradeSequence(
  ...
  const matches: { expectedIndex: number; time: number }[] = [];
  ...
  for (let i = 0; i < detected.length; i++) {
    ...
    if (matchedExpectedIndex !== undefined) {
      matches.push({ expectedIndex: matchedExpectedIndex, time: note.time });
      accepted.push(note);
      lastEcho = note;
      continue;
    }
