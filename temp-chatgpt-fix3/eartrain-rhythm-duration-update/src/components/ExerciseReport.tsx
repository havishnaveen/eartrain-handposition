...
function PerformanceReplay({
  ...
  const finishPlayback = useCallback(() => {
    ...
    onPlaybackEnd?.();
  }, [duration, onPlaybackEnd, onPlaybackFrame, plan, stopAnimation]);
  ...
  const updatePlayback = useCallback(() => {
    ...
    if (performance.now() - lastUiUpdateRef.current > 45 || capped >= duration) {
  ...
  }, [duration, finishPlayback, onPlaybackFrame, plan, recordingUrl]);
  ...
  const togglePlayback = useCallback(() => {
    ...
    setIsPlaying(true);
    animationRef.current = requestAnimationFrame(updatePlayback);
  }, [currentTime, duration, isPlaying, onPlaybackFrame, plan, recordingUrl, stopAnimation, updatePlayback]);
  ...
  const progress = Math.min(1, currentTime / Math.max(0.01, duration));
    ...
    <section className="et-replay" aria-labelledby="et-replay-title">
      ...
      <div className="et-replay__score">
        ...
        <div className="et-replay__note-lane" aria-label="Notes detected in your performance">
          ...
          <span className="et-replay__lane-playhead" style={{ left: `${progress * 100}%` }} />
        </div>
      </div>
      ...
      <div className="et-replay__transport">
        ...
        <div className="et-replay__waveform" aria-hidden="true">
          <span className="et-replay__waveform-progress" style={{ width: `${progress * 100}%` }} />
  ...
  return (
    <section className={`et-report et-report--${result.passed ? 'success' : 'focus'}`} aria-labelledby="exercise-report-title">
      ...
      <div className="et-report__scores" aria-label="Score breakdown">
        <ScoreMeter label="Pitch" value={result.scores.pitch} axis="pitch" />
        <ScoreMeter
          label="Timing"
          value={result.scores.timing}
          axis="timing"
          message={result.rhythm?.durationAccuracy !== null && result.rhythm?.durationAccuracy !== undefined
            ? `Attack timing and written note lengths were graded · ${Math.round(result.rhythm.durationAccuracy * 100)}% length accuracy.`
            : undefined}
        />
        <ScoreMeter label="Cleanliness" value={result.scores.cleanliness} axis="cleanliness" />
      </div>
