...
export function useDrillAudio(options: UseDrillAudioOptions = {}): DrillAudio {
  ...
  const { workletUrl = '/audio/pitch-processor.js?v=curriculum-audit-2026-08-13' } = options;
  ...
  const scheduleSpatialContext = useCallback((ctx: AudioContext, spec: SpatialChordSpec): number => {
    ...
    const schedulePiano = (pitches: string[], time: number, duration: number) => {
      pitches.forEach((pitch, noteIndex) => {
        [
          ...
          pan.pan.value = (noteIndex - (pitches.length - 1) / 2) * 0.08;
    ...
    let endTime = start + sequence.length * chordDuration;
    if (spec.rootSupport === 'matched') {
      // Give a clean, centered pitch-matching reference after the contextual
      // mix. The learner still has to retain and locate it on the keyboard,
      // but never needs perfect pitch or blind chord-name guessing.
      const anchorStart = endTime + 0.24;
      schedulePiano([spec.rootPitch], anchorStart, 0.62);
      schedulePiano([spec.rootPitch], anchorStart + 0.78, 0.62);
      endTime = anchorStart + 1.46;
    }
    window.setTimeout(() => {
