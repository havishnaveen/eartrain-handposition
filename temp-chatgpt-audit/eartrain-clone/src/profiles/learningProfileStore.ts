...
class LearningProfileStore {
  ...
  private ensureSession(sourceApp: PracticeSessionRecord['sourceApp']): PracticeSessionRecord {
    const now = Date.now();
  ...
  activateResolvedLaunch(launch: ResolvedStudentLaunch): LearnerProfile {
    ...
    this.ensureSession(launch.sourceApp);
    return student;
  }
  ...
  getSyncBatch(limit = 50): SyncAttemptEnvelope[] {
    const envelopes: SyncAttemptEnvelope[] = [];
    for (const event of this.state.outbox) {
      if (event.type !== 'learning.attempt.recorded') continue;
