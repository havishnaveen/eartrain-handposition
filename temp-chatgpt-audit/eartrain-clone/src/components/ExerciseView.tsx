...
export const ExerciseView = forwardRef<ExerciseViewHandle, ExerciseViewProps>(
  function ExerciseView(
    ...
    if (exerciseMode === 'spatial-chord' && spatialChord) {
      ...
      const previewRoot =
        spatialChord.rootSupport === 'shown' && isRootSearch && spatialProgress === 0;
      const staffPitches = previewRoot ? [spatialChord.rootPitch] : foundPitches;
      const buildingOuter = isShapeSearch && spatialProgress === 1;
        ...
        <section className={`et-spatial et-spatial--${status}`} aria-live="polite">
          ...
          <header className="et-spatial__header">
            ...
            <h2>
              {isCue
                ...
                : isShapeSearch
                  ? buildingOuter ? 'Make the outside shape' : 'Add the middle tone'
                  : spatialChord.rootSupport === 'shown' ? 'Place the anchor' : 'Match the anchor'}
            </h2>
            <p>
              {isCue
                ? spatialChord.rootSupport === 'matched'
                  ? 'Hear the mix. Then remember the two clear anchor notes.'
                  : 'Follow the clear piano sound.'
                : isShapeSearch
                  ? buildingOuter
                    ? `Keep Finger ${rootFinger} still. Reach Finger ${outerFinger}.`
                    : 'Keep the outside still. Place Finger 3 in the middle.'
                  : spatialChord.rootSupport === 'shown'
                    ? `Play ${rootName} with Finger ${rootFinger}.`
                    : `Play the same note with Finger ${rootFinger}.`}
            </p>
            {orientationNotice ? (
              <OrientationCallout
                notice={orientationNotice}
          ...
          <div className="et-spatial__body">
            ...
            <div className="et-spatial__staff" aria-label="Chord tones found on the staff">
              ...
              {staffPitches.map((pitch) => {
                const isPreview = previewRoot && pitch === spatialChord.rootPitch;
                return (
                <span
                  className={`et-spatial__staff-note${isPreview ? ' et-spatial__staff-note--preview' : ''}`}
                  key={`staff-${pitch}`}
                  style={{
              ...
              {staffPitches.length === 0 ? (
                <span className="et-spatial__staff-empty">
                  {spatialChord.rootSupport === 'matched' ? 'Match the anchor you heard' : 'Place the shown anchor'}
                </span>
              ) : null}
            </div>

            <div className="et-spatial__tones" aria-label="Chord tones found">
              {orderedToneEntries.map(({ pitch, chordIndex, stepIndex, finger }) => {
                const midi = pitchToMidi(pitch);
                const found = spatialFoundMidi.length > 0
                  ? midi !== null && spatialFoundMidi.includes(midi)
                  : spatialProgress > stepIndex;
                const root = chordIndex === 0;
                const shownBeforeLock = root && spatialChord.rootSupport === 'shown' && isRootSearch;
                const active = isRootSearch ? root : isShapeSearch && stepIndex === spatialProgress;
                return (
                  <div
                    key={pitch}
                    className={`et-spatial__tone${root ? ' et-spatial__tone--root' : ''}${found ? ' et-spatial__tone--found' : ''}${active ? ' et-spatial__tone--active' : ''}`}
                  >
                    <span>{root ? 'Anchor' : chordIndex === 2 ? 'Outside' : 'Middle'} · Finger {finger}</span>
                    <strong>{found || shownBeforeLock ? proofPitchName(pitch) : '?'}</strong>
            ...
            <div className="et-spatial__action">
                ...
                <div className="et-spatial__recording" role="status">
                  ...
                  <div>
                    <strong>
                      {isRootSearch
                        ? spatialChord.rootSupport === 'shown'
                          ? `Play ${rootName} · Finger ${rootFinger}`
                          : `Match the anchor · Finger ${rootFinger}`
                        : buildingOuter
                          ? `Reach Finger ${outerFinger}`
                          : 'Place Finger 3'}
                    </strong>
                    <small>
                      {spatialWrongGuesses > 0
              ...
              {isShapeSearch && foundPitches.length > 0 ? (
                <span className="et-spatial__root-lock">Anchor locked: {rootName}</span>
              ) : null}
            </div>
          </div>
        </section>
