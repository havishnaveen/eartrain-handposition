...
function ScoreMeter({
  ...
  return (
    <article className={`et-report-score${value === null ? ' et-report-score--na' : ''}`}>
      ...
      {result.transition ? (
        <section className="et-transition-result" aria-label="Hand shift result">
          <div>
            ...
            <h3>{result.transition.measured ? 'Hand movement measured' : 'Movement not captured'}</h3>
            <span>
              {result.transition.measured
                ? `${result.transition.transitionSeconds?.toFixed(2)} seconds between anchor notes · ${result.transition.excessBeats?.toFixed(2)} extra beats`
                : 'Play the last note before the arrow and the first note after it so the movement can be timed.'}
            </span>
          </div>
