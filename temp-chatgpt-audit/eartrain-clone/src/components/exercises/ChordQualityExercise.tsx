...
const CHORD_TYPES = [
  { name: "Major", pattern: CHORD_PATTERNS.major },
  { name: "Minor", pattern: CHORD_PATTERNS.minor },
  { name: "Augmented", pattern: CHORD_PATTERNS.augmented },
  { name: "Diminished", pattern: CHORD_PATTERNS.diminished },
];

export function ChordQualityExercise({ onAnswer, showHint , onNext}: any) {
    ...
    <div className="p-8 flex flex-col items-center relative overflow-hidden">
      ...
      {showHint && !isRetry && (
        <p className="text-orange-600 dark:text-orange-400 bg-orange-500/10 dark:bg-orange-500/10 p-4 rounded-xl text-sm font-medium mb-8 max-w-lg text-center">
          Hint: Major sounds happy/bright. Minor sounds sad/dark. Augmented sounds dreamy/unresolved/floating. Diminished sounds tense/scary/crunchy.
        </p>
      )}
