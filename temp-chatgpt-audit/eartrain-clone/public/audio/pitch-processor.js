 ...
 */
  ...
  flushPending(force) {
    for (let i = this.pending.length - 1; i >= 0; i--) {
      ...
      if (item.estimates.length < RECOVERY_MIN_ESTIMATES) {
        ...
        if (fallback) {
          this.port.postMessage({
            ...
            hypotheses,
            recoveryReason: 'estimator-disagreement',
          });
          continue;
        }
        this.port.postMessage({
      ...
      const reportedMidi = strict ? strictMidi : stableMidi;
      ...
      this.port.postMessage({
        ...
        pitchMad,
        hypotheses,
        recoveryReason: item.candidateOnly ? 'soft-attack' : 'pitch-confidence',
      });
    }
  }
  ...
  process(inputs) {
    ...
    if (this.listening && this.fluxCount >= 8) {
      ...
      const recoveryThreshold = Math.max(baseThreshold * 0.82, this.postOnset);
      ...
      if (
        this.armed &&
        (isPeak || isRecoveryPeak) &&
        ((loudEnough && deliberateAttack) || (recoverableLevel && recoverableAttack)) &&
        ...
        this.pending.push({
          ...
          frameAttackRatio,
          novelty: this.novelty1,
          candidateOnly: !(isPeak && loudEnough && deliberateAttack),
          estimates: [],
          yinEstimates: [],
          emergentEstimates: [],
        });
