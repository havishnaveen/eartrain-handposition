...
export function isClearSamePitchRetrigger(
  previous: Pick<RetriggerEvidence, 'time' | 'peakRms' | 'candidate'>,
...
export function priorProofKeysStillHeld(
  ...
  return proof.targetMidi.slice(0, proof.nextIndex).every((targetMidi) => {
...
export function useDrillAudio(options: UseDrillAudioOptions = {}): DrillAudio {
  ...
  const { workletUrl = '/audio/pitch-processor.js?v=score-aware-pcm-v13-2026-08-22' } = options;
  ...
  }, [stopLoops, stopSpatialPlayback, cancelMicRecording, cancelPcmCapture]);
      ...
      worklet.port.onmessage = (event: MessageEvent) => {
        ...
        if (data.type === 'note-release') {
          ...
          if (note) {
            ...
            if (data.reason !== 'reattack' && lastStrike?.detectorId === id) {
              lastStrikeByMidiRef.current.delete(note.midi);
            }
          }
          return;
        }
        ...
        if (data.type === 'note-onset' || data.type === 'note-candidate') {
          ...
          if (
            ...
            data.peakRms < data.gate * (isCandidate ? 0.6 : 1)
          ) {
            if (isCandidate) diagnosticsRef.current.candidatesIgnored += 1;
            return;
          }
          if (Number.isFinite(data.clarity) && data.clarity < (isCandidate ? 0.18 : 0.25)) return;
          if (Number.isFinite(data.consensus) && data.consensus < (isCandidate ? 0.42 : 0.5)) return;
          if (
            isCandidate &&
            Number.isFinite(data.pianoAttackConfidence) &&
            data.pianoAttackConfidence < 0.38
          ) {
  ...
  }, [stopLoops, stopSpatialPlayback, cancelMicRecording, cancelPcmCapture, safeSet]);
