...
export function ExerciseReport({
  ...
  // Melody reports stay intentionally student-sized. Detailed detections,
  // release timing, recording data, and shift telemetry are still retained
  // for persistence and the future teacher dashboard; they are merely not
  // rendered on this child-facing screen.
  const compactStudentReport = result.spatialChord == null;
    ...
    <section className={`et-report et-report--${result.passed ? 'success' : 'focus'}`} aria-labelledby="exercise-report-title">
      ...
      <div className="et-report__scores" aria-label="Score breakdown">
        <ScoreMeter
          ...
          axis="pitch"
          compact={compactStudentReport}
          message={result.spatialChord
            ? `${result.spatialChord.foundTones} of ${result.spatialChord.totalTones} chord tones found.`
            : undefined}
        />
        <ScoreMeter
          label="Timing"
          ...
          compact={compactStudentReport}
          message={result.spatialChord
            ? result.spatialChord.rootLatencySec === null
              ? 'The supplied anchor was not placed before the listening window ended.'
              : result.spatialChord.shellLatencySec === null
                ? 'Anchor placed. The outer fifth was not completed.'
                : `Outer reach ${result.spatialChord.shellLatencySec.toFixed(1)}s${result.spatialChord.colorToneLatencySec === null ? '' : ` · middle tone ${result.spatialChord.colorToneLatencySec.toFixed(1)}s`}.`
            : result.rhythm?.durationAccuracy !== null && result.rhythm?.durationAccuracy !== undefined
              ? `${result.rhythm.releaseEvaluated > 0 ? 'Key holds' : 'Attack spacing'} and timing were graded · ${Math.round(result.rhythm.durationAccuracy * 100)}% length accuracy.`
              : undefined}
        />
        <ScoreMeter
          label="Cleanliness"
          ...
          compact={compactStudentReport}
          message={result.spatialChord
            ? result.spatialChord.wrongRootGuesses + result.spatialChord.wrongShapeGuesses === 0
              ? 'Every movement followed anchor → outside → middle.'
              : `${result.spatialChord.wrongShapeGuesses} misplaced shape ${result.spatialChord.wrongShapeGuesses === 1 ? 'note' : 'notes'} · ${result.spatialChord.wrongRootGuesses} anchor retries.`
            : undefined}
        />
      </div>

      {!compactStudentReport && result.transition ? (
        <section className="et-transition-result" aria-label="Hand shift result">
          <div>
            <p className="et-report__section-kicker">Anchor &amp; shift</p>
            <h3>
              {result.transition.measured
                ? result.transition.onTime ? 'You landed in time' : 'Make the move a little sooner'
                : 'Play both sides of the purple marker'}
            </h3>
            <span>
              {result.transition.measured
                ? result.transition.onTime
                  ? `${result.transition.transitionSeconds?.toFixed(2)} seconds from the old position to the new one.`
                  : `${result.transition.transitionSeconds?.toFixed(2)} seconds to move. Shape the landing before your hand arrives.`
                : 'The microphone needs the note before the move and the landing note to time your shift.'}
            </span>
          </div>
          <p><strong>{result.transition.score.toFixed(1)}</strong><span>/5</span><small>Shift</small></p>
        </section>
      ) : null}

      {compactStudentReport ? null : (
        <PerformanceReplay
          result={result}
          expectedMusic={expectedMusic}
          plan={plan}
